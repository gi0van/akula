...
impl HeaderDownload {
    ...
    async fn collect_headers(
        ...
        match verify_chunks(&mut headers) {
            0 => (),
    ...
    async fn evaluate_chain_tip(
        ...
        let prevalidate_block_hashes = #[inline(always)]
        |s: &mut Self, hashes: &[BlockHashAndNumber]| -> bool {
            !hashes.is_empty()
...
#[inline]
fn verify_chunks(headers: &mut [BlockHeader]) -> usize {
