...
pub type InboundStream = futures_util::stream::SelectAll<SentryStream>;
...
impl PeerExt for Peer {
        ...
        let _ = self
            ...
            .map(|conn| {
                ...
                tokio::spawn(async move {
                    ...
                    if let Result::<_, anyhow::Error>::Err(err) = try {
                    ...
                    if let Result::<_, anyhow::Error>::Err(err) = try {
                        conn.set_status(tonic::Request::new(status_data)).await?;
                    } {
    ...
    async fn send_message(&self, msg: Message, predicate: PeerFilter) -> anyhow::Result<()> {
        ...
        let send_msg = #[inline(always)]
        async move |mut conn: SentryClient,
    ...
    async fn send_body_request(&self, request: Vec<H256>) -> anyhow::Result<()> {
        ...
        self.send_message(request.into(), PeerFilter::Random(10))
            .await?;
    ...
    async fn send_header_request(&self, request: HeaderRequest) -> anyhow::Result<()> {
        self.set_status().await?;
        self.send_message(request.into(), PeerFilter::Random(50))
            .await?;
