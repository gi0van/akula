...
impl TryFrom<grpc_sentry::InboundMessage> for InboundMessage {
    ...
    fn try_from(value: grpc_sentry::InboundMessage) -> Result<Self, Self::Error> {
        let msg = match MessageId::try_from(match grpc_sentry::MessageId::from_i32(value.id) {
            ...
            MessageId::NewBlockHashes => Message::NewBlockHashes(
                <NewBlockHashes as Decodable>::decode(&mut value.data.as_slice())?,
            ),
            MessageId::NewBlock => Message::NewBlock(Box::new(<NewBlock as Decodable>::decode(
                &mut value.data.as_slice(),
            )?)),
            MessageId::Transactions => todo!(),
            MessageId::NewPooledTransactionHashes => Message::NewPooledTransactionHashes(
                <NewPooledTransactionHashes as Decodable>::decode(&mut value.data.as_slice())?,
            ),
            MessageId::GetBlockHeaders => Message::GetBlockHeaders(
                <GetBlockHeaders as Decodable>::decode(&mut value.data.as_slice())?,
            ),
            MessageId::GetBlockBodies => Message::GetBlockBodies(
                <GetBlockBodies as Decodable>::decode(&mut value.data.as_slice())?,
            ),
            MessageId::GetNodeData => todo!(),
            MessageId::GetReceipts => todo!(),
            MessageId::GetPooledTransactions => todo!(),
            MessageId::BlockHeaders => Message::BlockHeaders(<BlockHeaders as Decodable>::decode(
                &mut value.data.as_slice(),
            )?),
            MessageId::BlockBodies => Message::BlockBodies(<BlockBodies as Decodable>::decode(
                &mut value.data.as_slice(),
            )?),
